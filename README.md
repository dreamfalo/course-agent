...
│   ├── config.py                     #   Agent 全局配置 (DeepSeek/Chroma/MySQL)
│   ├── llm/
│   │   └── deepseek_client.py        #   DeepSeek 统一客户端 (BaseChatModel)
│   ├── agent/
│   │   ├── course_agent.py           #   Agent 主调度器 (Tool 注册 + AgentExecutor)
│   │   └── memory_manager.py         #   ConversationBufferMemory + MySQL 持久化
│   ├── tools/
│   │   ├── schedule_tool.py          #   ScheduleTool — 课表 CRUD + 冲突校验 + 导出
│   │   ├── rag_retrieve_tool.py      #   RAGRetrieveTool — 文档向量化 + 语义检索
│   │   ├── task_plan_tool.py         #   TaskPlanTool — 自动生成学习计划
│   │   ├── file_manage_tool.py       #   FileManageTool — MinIO 文件管理
│   │   ├── note_tool.py              #   NoteTool — 笔记增删改查
│   │   ├── stat_tool.py              #   StatTool — 学习数据统计 + 折线图
│   │   └── config_tool.py            #   ConfigTool — 系统配置 + 用户管理
│   ├── auth/
...
│   └── tests/                        #   7 个单元测试文件
│
├── course_agent_backend/             # 🐍 Django 4.2 后端
│   ├── settings.py                   #   MySQL + JWT + MinIO + CORS + Chroma
│   ├── urls.py                       #   路由聚合
│   ├── exceptions.py                 #   全局异常捕获
│   ├── pagination.py                 #   统一分页
│   ├── minio_utils.py                #   MinIO 上传/下载工具类
│   └── wsgi.py / asgi.py             #   WSGI / ASGI 入口
│
├── schedule/                         # 📅 课表模块 (Django App)
│   ├── models.py                     #   UserSchedule ORM
│   ├── views.py                      #   CRUD + PDF教务系统解析 + CSV导入 + 导出
│   ├── serializers.py                #   DRF 序列化器
│   └── urls.py                       #   /api/schedule/
│
├── knowledge/                        # 📚 知识库模块 (Django App)
│   ├── models.py                     #   UserKnowledgeFile ORM
│   ├── views.py                      #   文件上传 / 向量化 / 检索 / 预览
│   ├── serializers.py
│   └── urls.py                       #   /api/knowledge/
│
├── task/                             # 📊 学习任务模块 (Django App)
│   ├── models.py                     #   UserTask ORM
│   ├── views.py                      #   任务CRUD + 甘特图 + AI生成
│   ├── serializers.py
│   └── urls.py                       #   /api/task/
│
├── agent_chat/                       # 💬 AI 对话模块 (Django App)
│   ├── models.py                     #   AgentChatHistory ORM
│   ├── views.py                      #   /api/agent/chat (调用 LangChain Agent)
│   ├── serializers.py
│   └── urls.py
│
├── config/                           # ⚙️ 系统配置模块 (Django App)
│   ├── models.py                     #   SystemConfig + SystemUser ORM
│   ├── views.py                      #   API密钥 / 向量库参数 / 账号管理
│   ├── serializers.py
│   └── urls.py
│
...
│   ├── src/
│   │   ├── views/                    # 8 个页面组件
│   │   │   ├── HomeView.vue          #   首页 (学习笔记 + 周动态折线图)
│   │   │   ├── ScheduleView.vue      #   课表管理 (周/月视图 + 上传 + 导出)
│   │   │   ├── ChatView.vue          #   AI 对话 (历史列表 + 气泡 + 发送)
│   │   │   ├── KnowledgeView.vue     #   知识库 (文件网格 + 批量上传)
│   │   │   ├── TasksView.vue         #   学习任务甘特图 (筛选 + AI生成)
│   │   │   ├── SettingsView.vue      #   系统配置 (模型参数 + 账号)
│   │   │   ├── LoginView.vue         #   登录页 (JWT 鉴权)
│   │   │   └── NotFoundView.vue      #   404
│   │   ├── components/               # 7 个公共组件
│   │   │   ├── BlueButton.vue        #   品牌蓝主按钮
│   │   │   ├── CalendarWidget.vue    #   日历 (周/月视图)
│   │   │   ├── ChatBubble.vue        #   对话气泡 (AI白/用户蓝)
│   │   │   ├── CourseCard.vue        #   课程卡片
│   │   │   ├── FilterSelect.vue      #   筛选下拉框
│   │   │   ├── GanttChart.vue        #   甘特图模块
│   │   │   └── LineChart.vue         #   折线图组件
│   │   ├── layouts/Layout.vue        #   全局三栏布局 + 渐变导航
│   │   ├── router/index.ts           #   Vue Router (8 路由 + 鉴权守卫)
│   │   ├── stores/                   #   Pinia 状态管理
│   │   │   ├── app.ts                #   全局状态 (学期/课程/模型参数)
│   │   │   └── user.ts               #   用户状态 (Token/用户信息)
│   │   ├── utils/request.ts          #   Axios 封装 (JWT 拦截)
│   │   └── styles/global.css         #   全局样式 (主色 #2981fd)
│   ├── package.json
│   ├── tsconfig.json
│   ├── vite.config.ts
│   ├── index.html
...
│
├── sql/
│   └── init_tables.sql               # MySQL 8.0 建表 SQL (Navicat 可直接执行)
│
├── tests/
│   └── full_chain_test.py            # 全链路集成测试
│
├── media/                            # 本地文件存储 (开发环境)
│
